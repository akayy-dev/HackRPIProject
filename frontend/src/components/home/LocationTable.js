...
import { fetchPlaceInfo } from "../../services/locationService";

const LocationTable = ({ nearbyPlaces, setNearbyPlaces }) => {
	...
	const toggleRow = async (index) => {
		...
		if (!nearbyPlaces[index].description) {
			...
			try {
				...
				console.error(
					`Error fetching description for ${nearbyPlaces[index].name}:`,
					error
				);
			} finally {
